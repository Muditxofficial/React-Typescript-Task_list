...
export default (state = initialState, action: ListsAction): ListState => {
  ...
  switch (action.type) {
    ...
    case ADD_TASK:
      ...
      copiedListsFromLS4[action.payload.list.id].tasks.push(
        action.payload.task
      );
    ...
    case DELETE_TASK:
      ...
      const copiedTasks = [
        ...copiedListsFromLS5[state.taskToDelete!.list.id].tasks,
      ];
      const task = copiedTasks.find(
