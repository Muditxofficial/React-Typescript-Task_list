//These are all types we are going to use in our app,
...
//interfaces for our tasks and lists.
...
/**We will store our lists inside local storage as an object
 *  which will have keys(id of the list)
 *  and values(the actual list). So we need Lists interface for that.
 *  id will be dynamic that’s why we need to use []. */
export interface Lists {
  [id: string]: List;
}

//For each action we also need to create an interface, each action will return type and payload.
...
//let’s create intefaces for our states.
//We will have 2 states, one for list and one for notification.
